...
contract Ticket_NFT is AccessControl, ERC1155Supply{
    ...
    event OrganizationCreatedTi(string organizationName, address admin);

    constructor(string memory _organization_name, address _organizer) ERC1155("") ERC1155Supply(){
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender); //quello chedeploya la blockchain per primo (io)
        ...
        //_setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE); //guarda perchè

        organization_name=_organization_name;
        admin = _organizer;

        emit OrganizationCreatedTi(organization_name, admin);
    }

    function getOrganizationDetails() external view returns (string memory, address) {
        return (organization_name, admin);
    }
    ...
    function createSector(uint _eventId, string memory _name, uint _totalSeats, uint _seat_x_lines) external onlyRole(ORGANIZER_ROLE) {  
        ...
        require(bytes(_event.name).length > 0, "Event does not exist");
    ...
    // mancano getter per tutti i ticket di un evento e per tutti i ticket di un utente, ma farlo lato front end è troppo
