...
const contract_marketplace_ABI = [
	...
	},
	{
		...
		"outputs": [
			...
			{
				"internalType": "enum Marketplace.TicketStatus",
				"name": "status",
				"type": "uint8"
			},
			{
...
const contract_marketplace_Address = '0x29eec5a1599740BCaE968911518bCE6Cde13286A';
...
export class MarketplaceService {
  ...
  private async initializeContract(): Promise<void> {
	  try {
		...
		this.contract = new ethers.Contract(contract_marketplace_Address, contract_marketplace_ABI, await signer);
	  } catch (error) {
  ...
  getContractAddress(): string {
    return contract_marketplace_Address; 
  }
	...
	async sellTicket(ticketId: number): Promise<void> {
		...
		for (let organizerId = 0; organizerId < nextNFTId; organizerId++) {
		  try {
			...
			const balance = await contract['balanceOf']((await signer).getAddress(), ticketId);
			if (balance > 0) {
